...
class Partner(models.Model):
    ...
    def _get_formatted_numbers(self):
        for partner in self:
            if partner.mobile:
                partner.mobile_format = re.sub(r'[^0-9\+]+', '', partner.mobile)
            else:
                partner.mobile_format = ''
            if partner.mobile_1:
                partner.mobile_1_format = re.sub(r'[^0-9\+]+', '', partner.mobile_1)
            else:
                partner.mobile_1_format = ''
            if partner.phone:
                partner.phone_format = re.sub(r'[^0-9\+]+', '', partner.phone)
            else:
                partner.phone_format = ''
            if partner.phone_1:
                partner.phone_1_format = re.sub(r'[^0-9\+]+', '', partner.phone_1)
            else:
